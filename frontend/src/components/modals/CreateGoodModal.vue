...
export default {
  ...
  computed: {
    category() {
      return this.categoryById(this.$store.state.createCategoryId) || {};
    },
    showModal: {
      get() {
        return this.value;
  ...
  methods: {
    async create() {
      ...
      if (response == {}) return
      this.$store.commit("setCreateGoodsList", [
        ...this.$store.state.createGoodsList,
        response.data,
      ]);
      this.$store.commit("setCreateGoodId", response.data.id);
      this.$store.commit("setShowSnackbar", true);
      this.$store.commit("setSnackbarText", "Товар успешно создан");
      this.creating = true;
