  ...
  <v-dialog v-model="showModal" width="600">
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn @click="save" text color="primary" :loading="saving">Сохранить</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    async save() {
      ...
      let response = await http.updateItem("Good", this.good.id, this.good, true);
      if (response == {}) return
      this.$store.commit("setShowSnackbar", true);
      this.$store.commit("setSnackbarText", "Товар успешно обновлен");
      this.saving = false;
