<template>
  <v-card class="h-100 border border-success" rounded outlined :to="router_link" @click="$emit('click')">
    ...
    <v-card-text>
      ...
      <v-text-field outlined rounded label="Цена" v-if="allow_edit_price" v-model="price" @blur="updatePrice"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    async updatePrice() {
      ...
      let response = await http.createItem(`/api/good/${this.good.id}/create_for_company/`, {
        price: this.price,
        user_id: this.$store.state.user.id,
      }, true);
      if (response == {}) return
      this.$store.commit("setShowSnackbar", true);
      this.$store.commit("setSnackbarText", "Цена успешно обновлена");
    }
  },
};
</script>
